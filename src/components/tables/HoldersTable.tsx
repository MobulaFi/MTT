...
import { truncate, formatPercentage, buildExplorerUrl, formatPureNumber, buildNativeSymbol } from '@mobula_labs/sdk';
import { getTokenAge } from '@/utils/Formatter';
import { ExternalLink, ArrowUpDown, ChevronUp, ChevronDown, Funnel, X, Building2, Lock } from 'lucide-react';
...
function formatExtraValue(key: string, value: unknown, decimals?: number): string {
  if (value === null || value === undefined) return '—';
  if (typeof value === 'boolean') return value ? 'Yes' : 'No';
  if (key === 'decimals') return String(value);
  if (key.endsWith('TimestampMs') && typeof value === 'number') {
    return new Date(value).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
  }
  if (key.endsWith('Raw') && typeof value === 'string') {
  ...
  if (key === 'periodSeconds' && typeof value === 'number') {
...
const EXTRA_LABELS: Record<string, string> = {
  ...
  endTimestampMs: 'End',
  cliffTimestampMs: 'Cliff',
  startTimestampMs: 'Start',
  amountPerPeriodRaw: 'Per Period',
  depositedAmountRaw: 'Deposited',
};
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-amber-500/20 border border-amber-500/30 ${
          ...
          {!compact && displayName}
          {extra && Object.keys(extra).length > 0 && <Lock size={compact ? 8 : 10} className="text-amber-300" />}
        </span>
      </TooltipTrigger>
      <TooltipContent side="top" className="text-[10px] max-w-[260px]">
          ...
          {extra && Object.keys(extra).length > 0 && (
            <div className="border-t border-border pt-1.5 flex flex-col gap-1">
              {Object.entries(extra)
                .filter(([key]) => key !== 'decimals')
                .map(([key, value]) => (
                <div key={key} className="flex items-center justify-between gap-3">
                  <span className="text-grayGhost">{EXTRA_LABELS[key] || key}</span>
                  <span className="text-white font-medium">{formatExtraValue(key, value, typeof extra.decimals === 'number' ? extra.decimals : undefined)}</span>
                </div>
              ))}
            </div>
          )}
...
const HolderRow = memo(function HolderRow({
  ...
  holder: TokenPositionsOutputResponse;
  index: number;
  totalSupply: number;
  tokenPrice: number;
  blockchain: string;
  ...
  const remainingPercent = totalSupply > 0 ? (tokenAmount / totalSupply) * 100 : 0;
  ...
  return (
    ...
    >
      ...
      <td className="px-1">
        <Funnel color="#777A8C" size={12} className="cursor-pointer hover:opacity-70 transition-opacity" />
      </td>
      <td className="text-left whitespace-nowrap px-2">
        <div className="flex items-center gap-1.5">
          {holder.walletAddress && blockchain && (() => {
            ...
            return explorerUrl ? (
              <a href={explorerUrl} target="_blank" rel="noopener noreferrer" className="text-grayGhost hover:text-textPrimary transition-colors" title="View on explorer">
          ...
          {holder.labels?.includes('liquidityPool') ? (
            ...
            <span
              onClick={() => useWalletModalStore.getState().openWalletModal({ walletAddress: holder.walletAddress, txHash: holder.walletAddress, blockchain })}
              className="text-accentPurple hover:underline-offset-2 hover:underline cursor-pointer truncate max-w-[100px]"
            >
              {truncate(holder.walletAddress, { length: 4, mode: 'middle' })}
            </span>
          )}
          <WalletEntityBadge metadata={(holder as typeof holder & { walletMetadata?: WalletMetadata }).walletMetadata} />
                ...
                if (prominentLabel) {
                  return (
                    <Tooltip key={tag}>
                      <TooltipTrigger asChild>
                        <button
                          onClick={(e) => { e.stopPropagation(); handleLabelClick(tag); }}
                          className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wide transition-all cursor-pointer ${prominentLabel.className} ${isActiveFilter ? 'ring-1 ring-white/30' : 'hover:brightness-110'}`}
                        >
                          {icon}
                          {prominentLabel.text}
                        </button>
                      ...
                      <TooltipContent side="top" className="text-[10px]">
                        <span className="text-grayGhost">{isActiveFilter ? 'Click to clear filter' : 'Click to filter'}</span>
                      </TooltipContent>
                    </Tooltip>
                  );
                }
                return icon ? (
                  <Tooltip key={tag}>
                    <TooltipTrigger asChild>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleLabelClick(tag); }}
                        className={`transition-all cursor-pointer ${isActiveFilter ? 'opacity-100 ring-1 ring-accentPurple/50 rounded' : 'opacity-70 hover:opacity-100'}`}
                      >
                        {icon}
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="top" className="text-[10px]">
                      ...
                      <span className="text-grayGhost ml-1">{isActiveFilter ? '(click to clear)' : '(click to filter)'}</span>
                    </TooltipContent>
                  </Tooltip>
                ) : null;
              })}
      ...
      <td className="text-left px-2">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-grayGhost">{holder.lastActivityAt ? getTokenAge(holder.lastActivityAt) : '—'}</span>
        </div>
      </td>
      <td className="text-left px-2">
        <span className="text-white">
          {Number(holder.nativeBalance) > 0 ? (
            <>{formatPureNumber(Number(holder.nativeBalance))}<span className="text-[10px] text-grayGhost ml-1">{buildNativeSymbol(holder.chainId)}</span></>
          ) : (
      ...
      <td className="text-left px-2">
        <div className="flex flex-col">
          <span className="text-success"><PriceDisplay usdAmount={holder.volumeBuyUSD} align="left" /></span>
          <div className="flex items-center gap-1 text-[10px] text-grayGhost"><span>{holder.buys} / {holder.sells}</span></div>
        </div>
      </td>
      <td className="text-left px-2">
        <div className="flex flex-col">
          <span className="text-white"><PriceDisplay usdAmount={holder.volumeSellUSD} align="left" /></span>
          <span className="text-[10px] text-grayGhost">{avgSellPrice > 0 ? <PriceDisplay usdAmount={avgSellPrice} align="left" /> : '—'}</span>
        </div>
      </td>
      <td className="text-left px-2">
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="flex flex-col cursor-help">
              ...
              <div className="flex items-center gap-2 text-[9px]">
                <span className={realizedPnlValue >= 0 ? 'text-success/70' : 'text-red-500/70'}>R: <PriceDisplay usdAmount={realizedPnlValue} align="left" /></span>
                <span className={unrealizedPnlValue >= 0 ? 'text-success/70' : 'text-red-500/70'}>U: <PriceDisplay usdAmount={unrealizedPnlValue} align="left" /></span>
              </div>
            </div>
          </TooltipTrigger>
          <TooltipContent side="top" className="text-[10px]">
            <div className="flex flex-col gap-1 min-w-[100px]">
              ...
              <div className="flex justify-between gap-4">
                ...
                <span className={realizedPnlValue >= 0 ? 'text-success' : 'text-red-500'}><PriceDisplay usdAmount={realizedPnlValue} align="right" /></span>
              </div>
              <div className="flex justify-between gap-4">
                <span className="text-grayGhost">Unrealized:</span>
                <span className={unrealizedPnlValue >= 0 ? 'text-success' : 'text-red-500'}><PriceDisplay usdAmount={unrealizedPnlValue} align="right" /></span>
              </div>
              <div className="flex justify-between gap-4 border-t border-borderDefault pt-1 mt-0.5">
                ...
                <span className={`font-medium ${totalPnlValue >= 0 ? 'text-success' : 'text-red-500'}`}><PriceDisplay usdAmount={totalPnlValue} align="right" /></span>
              </div>
            </div>
          </TooltipContent>
        </Tooltip>
        ...
        {(() => {
          ...
          return platform?.name ? (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-bgTertiary text-[10px] font-medium text-white">
              {platform.logo && <img src={platform.logo} width={12} height={12} alt={platform.name || ''} className="rounded-full" />}
              {platform.name}
            </span>
          ) : (
            <span className="text-grayGhost">—</span>
      ...
      <td className="px-2">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="text-grayGhost text-[10px] w-10">{formatPercentage(remainingPercent)}</span>
            <PriceDisplay usdAmount={balanceUSD} align="left" />
          </div>
          <div className="w-full bg-borderDefault rounded-full h-1 overflow-hidden">
            <div className="bg-success h-1 rounded-full transition-all duration-700" style={{ width: `${Math.min(remainingPercent, 100)}%` }} />
          </div>
        </div>
      </td>
    ...
    const sorted = [...holders].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'avgSell':
          ...
          break;
        case 'nativeBalance':
          aVal = Number(a.nativeBalance) || 0;
          bVal = Number(b.nativeBalance) || 0;
          break;
        default:
          aVal = 0;
          bVal = 0;
    ...
    <TooltipProvider>
      <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-thumb-[#22242D] scrollbar-track-transparent hover:scrollbar-thumb-[#343439]">
          <table className="min-w-[1000px] w-full text-xs bg-bgPrimary border-collapse">
            <thead className="text-grayGhost bg-bgPrimary h-9 sticky top-0 z-20 border-b border-borderDefault shadow-sm text-xs">
              <tr>
                ...
                <th className="w-[100px] whitespace-nowrap text-left px-2">
                  <SortableHeader
                    label="Native Bal."
                    field="nativeBalance"
                    sortField={sortField}
