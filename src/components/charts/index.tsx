...
const TradingViewChart = ({
  ...
  const initialFromRef = useRef<number | undefined>(initialFrom);
  const initialToRef = useRef<number | undefined>(initialTo);
      ...
      try {
        ...
        tvWidget.onChartReady(async () => {
          ...
          try {
            ...
            // Set visible time range if from/to provided
            if (initialFromRef.current && initialToRef.current) {
              try {
                chart.setVisibleRange({
                  from: initialFromRef.current,
                  to: initialToRef.current,
                });
              } catch (error) {
