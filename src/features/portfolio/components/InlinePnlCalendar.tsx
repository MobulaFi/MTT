...
let inlineCalendarRequestId = 0;

interface InlinePnlCalendarProps {
  walletAddress: string;
  blockchain?: string;
}

export function InlinePnlCalendar({ walletAddress, blockchain }: InlinePnlCalendarProps) {
  ...
  const fetchMonthData = useCallback(async (targetYear: number, targetMonth: number) => {
    ...
    // Cap to now so the API includes today's partial data
    const toDate = endOfMonth.getTime() > Date.now() ? new Date() : endOfMonth;
    const fetchKey = `${walletAddress}-${targetYear}-${targetMonth}-${formatDateKey(new Date())}`;

    if (lastFetchRef.current === fetchKey) return;

    inlineCalendarRequestId++;
    const thisRequestId = inlineCalendarRequestId;
    ...
    try {
      ...
      if (thisRequestId !== inlineCalendarRequestId) return;
      ...
      if (thisRequestId === inlineCalendarRequestId) {
